...
class PurchaseReturnController extends Controller
{
    public function index() {
        ...
        $purchaseReturns = PurchaseReturn::where('department_code', 'DP01')->orderBy('id','desc')->get();
        ...
        return view('transaction.purchase-return.purchase_return_list', compact('companies', 'departments', 'purchaseReturns', 'suppliers', 'suppliers', 'prices', 'taxs','privileges'));
    }
    ...
    public function summary(Request $request)
    {
        ...
        $purchaseInvoices = $query->where('department_code','DP01')->orderBy('document_date','asc')->orderBy('id','asc')->get();
        ...
        return view('transaction.purchase-return.purchase_return_summary', compact('purchaseInvoices', 'totalAmount','privileges'))
        ->with([
    ...
    public function printPDF($purchase_return_number)
    {
        ...
        return view('transaction.purchase-return.purchase_return_pdf', compact('purchaseInvoice','totalHuruf'));
    ...
    public function edit($id) {
        try {
            ...
            return view('transaction.purchase-return.purchase_return_edit', compact('purchaseReturn', 'companies',  'departments', 'coas', 'items', 'itemUnits','itemDetails', 'suppliers', 'prices', 'taxs','privileges'));
        } catch (\Exception $e) {
            dd($e);
    ...
    public function update(Request $request, $id)
    {
        ...
        try {
            ...
            $oldPrDetails = PurchaseReturnDetail::where('purchase_return_number',$purchaseReturn->purchase_return_number)->get();

            Journal::where('document_number',$purchaseReturn->purchase_return_number)->delete();
            InventoryDetail::where('document_number',$purchaseReturn->purchase_return_number)->delete();
            ...
            return redirect()->route('transaction.purchase_return')->with('success', 'Purchase Return edited successfully!');
        } catch (\Exception $e) {
    ...
    public function updateStatus(Request $request, $id)
    {
        ...
        try {
            ...
            // Parse and assign date fields
            DB::commit(); // Commit the transaction
            return redirect()->route('transaction.purchase_return')->with('success', 'APPROVED');;
        } catch (\Exception $e) {
            DB::rollBack(); // Rollback transaction on error
    ...
    public function destroy($id) {
        ...
        try {
            ...
            InventoryDetail::where('document_number',$general->purchase_return_number)->delete();
            ...
            return redirect()->route('transaction.purchase_return')->with('success', 'Purchase Invoice deleted successfully!');
        } catch (\Exception $e) {
            DB::rollBack();
            // dd($e);
            Log::error($e->getMessage());
            return redirect()->route('transaction.purchase_return')->with('error', 'Error deleting: ' . $e->getMessage());
        }
    }

    private function generatePurchaseReturnNumber() {
        $today = now();
    ...
    public function create(Request $request)
    {
        ...
        $privileges = Auth::user()->roles->privileges['purchase_return'];
        return view('transaction.purchase-return.purchase_return_input', compact(
            // 'salesOrders',
            'suppliers',
            'departments',
            'itemUnits',
    ...
    public function recalcJournal(){
        ...
        try {
            ...
            foreach ($purchaseReturns as $pr) {
                $docNumber = $pr->purchase_return_number;
                Journal::where('document_number',$docNumber)->delete();
                $purchaseReturn = PurchaseReturn::where('purchase_return_number',$docNumber)->first();
                $details = PurchaseReturnDetail::where('purchase_return_number',$docNumber)->get();
                ...
                foreach ($details as $key => $detail) {
                    ...
                    $PODJournal1->credit_nominal = $detail->nominal;
                ...
                $PODJournal1->notes ='HPP for item: '. $itemNameConcat;
                $PODJournal1->company_code = $purchaseReturn->company_code;
                ...
                $POJournal->notes = 'Return Purchase from '.$suppliers->supplier_name.' ('.$purchaseReturn->purchase_return_number.')';
                $POJournal->company_code = $purchaseReturn->company_code;
                $POJournal->department_code = $purchaseReturn->department_code;
                ...
                $POJournal2->notes = 'Return Add. tax on purchase from '.$suppliers->supplier_name.' ('.$purchaseReturn->purchase_return_number.')';
                $POJournal2->company_code = $purchaseReturn->company_code;
                $POJournal2->department_code = $purchaseReturn->department_code;
            ...
            DB::commit();
                return redirect()->route('transaction.purchase_return.create')->with('success', 'Purchase Return created successfully.');
        } catch (\Exception $e) {
            DB::rollBack();
            dd($e);
    ...
    {
        $exist = PurchaseReturn::where('token',$request->token)->where('department_code','DP01')->whereDate('created_at',Carbon::today())->first();
        if($exist){
            $id = PurchaseReturn::where('created_by',Auth::user()->username)->orderBy('id','desc')->select('id')->first()->id;
            return redirect()->route('transaction.purchase_return.create')->with('success', 'Purchase Return created successfully.')->with('id',$id);
        }
        // Generate a new purchase order number
        $purchaseReturnNumber = $this->generatePurchaseReturnNumber();
        // dd($request->all());
        DB::beginTransaction();
        try {
            ...
            $purchaseReturn->department_code = 'DP01';
...
// dd($purchaseReturn);
    ...
    public function recalculateInventory()
    {
        DB::beginTransaction();
        try {
            $details = PurchaseReturnDetail::with('header')->get();
            foreach ($details as $detail) {
                $firstQ = InventoryDetail::where('item_id', $detail->item_id)->orderBy('id','desc')->first();
                $firstQty = $firstQ->last_qty??0;
                $item = Item::where('item_code',$detail->item_id)->where('department_code',$detail->department_code)->first();
                $gd = Warehouse::where("warehouse_code",$item->warehouse_code)->first();
                $cogs = Module::getCogs($detail->header->document_date,$detail->item_id,$detail->header->company_code,$detail->header->department_code,$gd->id);
                InventoryDetail::insert([
                    'document_number'=>$detail->purchase_return_number,
                    'document_date'=>$detail->header->document_date,
                    'transaction_type'=>'Purchase Return',
                    'from_to'=>$detail->header->supplier_code,
                    'item_id'=>$detail->item_id,
                    'quantity'=>$detail->qty,
                    'unit'=>$detail->unit,
                    'base_quantity'=>$detail->base_qty,
                    'unit_base'=>$detail->base_unit,
                    'company_code'=>$detail->header->company_code,
                    'department_code'=>$detail->header->department_code,
                    'first_qty'=>$firstQty,
                    'last_qty'=>$firstQty-($detail->qty*$detail->base_qty),
                    'created_at' => Carbon::now('Asia/Jakarta'),
                    'updated_at' => Carbon::now('Asia/Jakarta'),
                    'created_by' => $detail->header->created_by,
                    'updated_by' => $detail->header->created_by,
                    'warehouse_id' => $gd->id,
                    'total' => $detail->nominal*-1,
