...
class PurchaseInvoiceRecapController extends Controller
{
    public function index() {
        $companies = Company::all();
        $departments = Department::where('department_code', 'DP01')->first();
        $purchaseInvoices = PurchaseInvoice::where('recap', 'yes')->with('debts')->orderBy('id','desc')->get();
        $coas = COA::all();
        ...
        return view('transaction.purchase-invoice-recap.purchase_invoice_list', compact('companies', 'departments', 'purchaseInvoices', 'coas',  'suppliers', 'suppliers', 'prices', 'taxs','privileges'));
    }
    ...
    public function cancel(Request $request, $id) {
        ...
        try {
            ...
            return redirect()->route('transaction.purchase_invoice_recap')->with('success', 'Purchase Invoice cancelled successfully!');
        } catch (\Exception $e) {
            DB::rollBack();
            // dd($e);
            Log::error($e->getMessage());
            return redirect()->route('transaction.purchase_invoice_recap')->with('error', 'Error canceling: ' . $e->getMessage());
        }
    }
    ...
    public function summary(Request $request)
    {
        ...
        return view('transaction.purchase-invoice-recap.purchase_invoice_summary', compact('purchaseInvoices', 'totalAmount'))
        ->with([
    ...
    public function summaryDetail(Request $request)
    {
        ...
        // Return the view with the purchase invoices data and total amount
        return view('transaction.purchase-invoice-recap.purchase_invoice_summary_detail', compact('purchaseInvoices', 'totalAmount'))
        ->with([
            'from_date' => $request->input('from_date'),
            'to_date' => $request->input('to_date')
        ]);
    ...
    public function printPDF($purchase_invoice_number)
    {
        ...
        return view('transaction.purchase-invoice-recap.purchase_invoice_pdf', compact('purchaseInvoice','totalHuruf','totalDiscount'));
    ...
    public function printPDFNetto($purchase_invoice_number)
    {
        ...
        // Generate and return PDF
        $totalHuruf = ucfirst($this->numberToWords($purchaseInvoice->total)).' rupiah';
        return view('transaction.purchase-invoice-recap.purchase_invoice_netto_pdf', compact('purchaseInvoice','tax','totalHuruf','totalDiscount'));
        $nameFile = Str::replace("/", "", $purchaseInvoice->purchase_invoice_number);
        return $pdf->stream("Purchase_Invoice_{$nameFile}.pdf");
    }
    ...
    public function edit($id) {
        try {
            ...
            $departments = Department::where('department_code', $purchaseOrder->department_code)->first();
            $coas = Coa::all();
            $items = ItemPurchase::where('department_code',$purchaseOrder->department_code)->orderBy('item_code')->distinct('item_code')->with('items', 'unitn','itemDetails','items.warehouses')->get();
            $itemUnits = ItemUnit::all();
            $itemDetails = ItemDetail::where('department_code',$purchaseOrder->department_code)->get();
            $suppliers = Supplier::where('department_code',$purchaseOrder->department_code)->get();
            $taxs = TaxMaster::all();
            ...
            return view('transaction.purchase-invoice-recap.purchase_invoice_edit', compact('purchaseOrder', 'companies', 'departments', 'coas', 'items', 'itemUnits','itemDetails', 'suppliers', 'suppliers', 'taxs', 'purchaseOrderDetails','editable','privileges'));
        } catch (\Exception $e) {
            dd($e);
    ...
    public function update(Request $request, $id)
    {
        ...
        try {
            ...
            InventoryDetailRecap::where('document_number',$purchaseInvoiceNumber)->delete();
            $this->savePurchaseInvoiceDetails($request->details,$purchaseOrder);

            DB::commit();
            return redirect()->route('transaction.purchase_invoice_recap')->with('success', 'Purchase Invoice created successfully.');
        } catch (\Exception $e) {
            DB::rollBack();
            // dd($e);
            Log::error($e);
    ...
    public function updateStatus(Request $request, $id)
    {
        ...
        try {
            ...
            return redirect()->route('transaction.purchase_invoice_recap')->with('success', 'APPROVED');;
        } catch (\Exception $e) {
    ...
    public function destroy(Request $request, $id) {
        ...
        try {
            ...
            InventoryDetailRecap::where('document_number',$general->purchase_invoice_number)->delete();
            ...
            DB::commit(); // Commit transaction
            return redirect()->route('transaction.purchase_invoice_recap')->with('success', 'Purchase Invoice deleted successfully!');
        } catch (\Exception $e) {
            DB::rollBack();
            // dd($e);
            Log::error($e->getMessage());
            return redirect()->route('transaction.purchase_invoice_recap')->with('error', 'Error deleting: ' . $e->getMessage());
        }
    }

    private function generatePurchaseInvoiceNumber($date,$department) {
        ...
        if($department=='DP02'){
            return "WIL/PUI/{$romanMonth}/{$year}-{$newNumber}";
        }else if($department=='DP03'){
            return "DRE/PUI/{$romanMonth}/{$year}-{$newNumber}";
        }else{
    ...
    public function changeDepartment(Request $request)
    {
        ...
        $items = ItemPurchase::where('department_code',$department)->orderBy('item_code')->distinct('item_code')->with('items', 'unitn','itemDetails','items.warehouses')->get();
        $itemDetails = ItemDetail::where('department_code',$department)->get();
        $prices = ItemPurchase::where('department_code',$department)->orderBy(
            'item_code',
            'asc'
        )->get();
    ...
    public function create(Request $request)
    {
        ...
        $items = ItemPurchase::where('department_code','DP01')->orderBy('item_code')->distinct('item_code')->with('items', 'unitn','itemDetails','items.warehouses')->get();
        $itemUnits = ItemUnit::all();
        ...
        $privileges = Auth::user()->roles->privileges['purchase_invoice'];
        return view('transaction.purchase-invoice-recap.purchase_invoice_input', compact(
            // 'salesOrders',
            'suppliers',
            'departments',
            'items',
    ...
    {
        $exist = PurchaseInvoice::where('token',$request->token)->where('department_code',$request->department_code)->whereDate('created_at',Carbon::today())->first();
        if($exist){
            $id = PurchaseInvoice::where('created_by',Auth::user()->username)->orderBy('id','desc')->select('id')->first()->id;
            return redirect()->route('transaction.purchase_invoice_recap.create')->with('success', 'Purchase Invoice created successfully.')->with('id',$id);
        }
        // Generate a new purchase order number
        $purchaseInvoiceNumber = $this->generatePurchaseInvoiceNumber($request->document_date,$request->department_code);
        DB::beginTransaction();
        try {
            // Generate a new PurchaseOrder instance
            ...
            // Finalize total
            $purchaseOrder->company_code = $request->company_code ?? null;
            $purchaseOrder->department_code = $request->department_code;
            $purchaseOrder->include = $request->include ?? false;
            // $purchaseOrder->save();
            $purchaseOrder->tax_revenue_tariff = $request->tax_revenue;
    ...
    private function savePurchaseInvoiceDetails(array $piDetails,$purchaseOrder){
        ...
        // Process purchase order details
            ...
            foreach ($piDetails as $detail) {
                ...
                $detail['department_code'] = $purchaseOrder->department_code;
                ...
                // dd($nominal);
                $item = ItemDetail::where('department_code',$purchaseOrder->department_code)->where([
                ...
                $itemUnit = ItemPurchase::where('department_code',$purchaseOrder->department_code)->where('item_code', $detail['item_id'])->first();
