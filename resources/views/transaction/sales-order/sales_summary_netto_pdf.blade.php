...
<body>
    ...
    TDS                    PERMINTAAN PENJUALAN NETTO
    Kepada Yth.
...
@if($lineIndex == 0)
    |{{ str_pad($loop->parent->iteration . '.', 3) }}|{{ str_pad($line, 35) }}|{{ str_pad(number_format($detail->qty, 0), 5, ' ', STR_PAD_LEFT) }}|{{ str_pad(number_format($detail->base_qty * $detail->qty, 2, ',', '.') . ' ' . $detail->baseUnit->unit_name, 11, ' ', STR_PAD_LEFT) }}|{{ str_pad($detail->items->additional_tax? number_format($detail->price + ($detail->price*$tax->tariff/100), 0,',','.'):number_format($detail->price, 0,',','.'), 12, ' ', STR_PAD_LEFT) }}|{{ str_pad($detail->items->additional_tax? number_format($detail->nominal + ($detail->price*$detail->qty*$tax->tariff/100), 0,',','.'):number_format($detail->nominal, 0,',','.'), 14, ' ', STR_PAD_LEFT) }}|
@else
...
@endphp
    {{ str_pad($terbilangLines[0], 35) }}                     Sub Total    : {{ str_pad(number_format($salesOrder->subtotal+$salesOrder->add_tax, 0,',','.'), 15, ' ', STR_PAD_LEFT) }}
