...
@section('content')
<div class="row">
    <x-page-title title="{{__('Tanda Terima')}}" pagetitle="{{__('Tanda Terima')}} Edit" />
    <hr>
    <div class="container content">
        <h2>{{__('Tanda Terima')}} Edit</h2>
        ...
        <form id="rl-form" action="{{ isset($receivableList) ? route('transaction.receivable_list.update', $receivableList->id) : route('transaction.receivable_list.insert') }}" method="POST">
                    ...
                    <div class="row">
                        <div class="col-md-4">
                            <div class="form-group">
                                ...
                                <input type="text" name="customer_code" id="customer_code" class="form-control" readonly required value="{{ old('customer_code', $receivableList->customer_code) }}">
                            </div>
                            <br>
                            <div class="form-group">
                                <label for="customer_name">{{__('Customer Name')}}</label>
                                <input type="text" name="customer_name" id="customer_name" class="form-control" readonly required value="{{ old('customer_name', $receivableList->customers->customer_name) }}">
                            </div>
                            <br>
                        </div>
            ...
            <div class="card mb-3">
                ...
                <div class="card-body">
                    <h5 class="text-end">Total: <span id="total-value">{{ number_format($receivableList->total,0,'.',',') }}</span></h5>
                        ...
                        <tbody id="parentTbody">
                            @foreach($receivableListDetails as $index => $details)
                                <tr>
                                    ...
                                    <td>
                                        <input type="date" name="details[{{ $index }}][document_date]" class="form-control date-picker" value="{{$details->document_date}}" readonly />
                                    </td>
                                    <td>
            ...
            <a href="{{route('transaction.receivable_list')}}" class="btn btn-secondary mb-3">Back</a>
        </form>

        @if($editable)
        <form id="delete-form" action="{{ route('transaction.receivable_list.delete', $receivableList->id) }}" method="POST" style="display:inline;" >
            @csrf
            @method('POST')
...
function confirmDelete(event, id) {
    ...
    Swal.fire({
        title: 'Are you sure?',
        text: 'Do you want to delete this Tanda Terima?',
        icon: 'warning',
