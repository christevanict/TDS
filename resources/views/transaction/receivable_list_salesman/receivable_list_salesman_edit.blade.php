...
@section('content')
<div class="row">
    <x-page-title title="{{__('Daftar Tagihan')}}" pagetitle="{{__('Daftar Tagihan')}} Edit" />
    <hr>
    <div class="container content">
        <h2>{{__('Daftar Tagihan')}} Edit</h2>
        ...
        <form id="rl-form" action="{{ isset($receivableList) ? route('transaction.receivable_list_salesman.update', $receivableList->id) : route('transaction.receivable_list_salesman.insert') }}" method="POST">
                    ...
                    <div class="row">
                        <div class="col-md-4">
                            <div class="form-group">
                                ...
                                <input type="text" name="city_code" id="city_code" class="form-control" readonly required value="{{ old('city_code', $receivableList->city_code) }}">
                            </div>
                        </div>
                        ...
                        <div class="col-md-4">
                            <div class="form-group">
                                ...
                                <input type="date" name="document_date" id="document_date" class="form-control date-picker" required value="{{ old('document_date', $receivableList->receivable_list_date ?? date('Y-m-d')) }}">
                            </div>
                        </div>
            ...
            <div class="card mb-3">
                <div class="card-header">{{__('Daftar Tagihan')}} Details</div>
                <div class="card-body">
                    ...
                    <div style="overflow-x: auto;">
                        <table class="table" id="dynamicTable">
                            ...
                            <tbody id="parentTbody">
                                @foreach($receivableListDetails as $index => $details)
                                    @php
                                        $date=date_create($details->document_date);
                                    @endphp
                                    <tr>
                                        <td>
                                            <input type="text" name="details[{{ $index }}][customer_code]" class="form-control" value="{{$details->customer_code_document}}" readonly />
                                        </td>
                                        <td>
                                            <input type="text" name="details[{{ $index }}][customer_name]" class="form-control" value="{{$details->customers->customer_name}}" readonly />
                                        </td>
                                        <td>
                                            <input type="text" class="form-control" value="{{date_format($date,'d-m-Y')}}" readonly />
                                            <input type="hidden" name="details[{{ $index }}][document_date]" class="form-control" value="{{$details->document_date}}" />
                                        </td>
                                        <td>
                                            <input type="text" name="details[{{ $index }}][document_number]" class="form-control" value="{{$details->document_number}}" readonly />
                                        </td>
                                        <td>
                                            <input type="text" name="details[{{ $index }}][nominal]" class="form-control nominal" value="{{ number_format($details->nominal,0,'.',',') }}" readonly />
                                        </td>
                                        <td>
                                            <input type="text" name="details[{{ $index }}][paid]" class="form-control nominal" value="{{ number_format($details->paid,0,'.',',') }}" readonly />
                                        </td>
                                        <td>
                                            <input type="text" name="details[{{ $index }}][nominal_left]" class="form-control nominal total" value="{{ number_format($details->nominal_left,0,'.',',') }}" readonly />
                                        </td>
                                        <td>
            ...
            @if($editable)
                <div class="form-group submit-btn mb-3">
                    <button type="submit" class="btn btn-primary" @if(!in_array('update', $privileges)) disabled @endif>Update </button>
                </div>
            @endif
            <a href="{{route('transaction.receivable_list_salesman')}}" class="btn btn-secondary mb-3">Back</a>
        </form>

        @if($editable)
        <form id="delete-form" action="{{ route('transaction.receivable_list_salesman.delete', $receivableList->id) }}" method="POST" style="display:inline;" >
            @csrf
            @method('POST')
            <input type="hidden" name="reason" id="deletion-reason">
            <button type="button" class="btn btn-sm btn-danger mb-3" onclick="confirmDelete(event,'{{ $receivableList->id }}')"
...
function confirmDelete(event, id) {
    ...
    Swal.fire({
        title: 'Are you sure?',
        text: 'Do you want to delete this Daftar Tagihan?',
        icon: 'warning',
