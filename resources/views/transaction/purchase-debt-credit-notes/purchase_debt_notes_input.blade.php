...
@section('title', 'Input Purchase Debit Note')
...
@section('content')
    <div class="row">
        <x-page-title title="Purchase Debit Note" pagetitle="Purchase Debit Note Input" />
        <hr>
        <div class="container content">
            <h2>Purchase Debit Note Input</h2>

            <form id="purchase-debt-note-form" action="{{ route('transaction.purchase_debt_credit_notes.store') }}" method="POST">
                @csrf
                <!-- Hidden input for status -->
                <input type="hidden" name="status" value="debit">
                <div class="card mb-3">
                    <div class="card-header">Purchase Debit Note {{__('Information')}}</div>
                    <div class="card-body">
                        <div class="row">
                            <div class="col-md-6">
                                <div class="form-group mt-3">
                                    <label for="purchase_credit_note_date">Debit Note Date</label>
                                    <input type="date" name="purchase_credit_note_date" id="purchase_credit_note_date"
                                ...
                                <div class="form-group mt-3">
                                    ...
                                    <select name="invoice_number" id="invoice_number" class="form-control" required>
                                        ...
                                        @foreach ($purchaseInvoices as $invoice)
                                            <option value="{{ $invoice->purchase_invoice_number }}">
                                                {{ $invoice->purchase_invoice_number }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
