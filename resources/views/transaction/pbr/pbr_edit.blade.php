...
<div class="row">
    ...
    <div class="container content">
        ...
        @if ($pbr->reason)
            <h5 style="color: red">Alasan edit: {{$pbr->reason}}</h5>
        @endif
        <form id="print-form" target="_blank" action="{{ route('pbr.print', $pbr->id) }}" method="GET"
            style="display:inline;">
            <button type="submit" class="mb-3 btn btn-dark" @if(!in_array('print', $privileges)) disabled @endif>
                Print</button>
        </form>

        <form id="contract-form" action="{{ isset($pbr) ? route('transaction.pbr.update', $pbr->id) : route('transaction.pbr.store') }}" method="POST">
            @csrf
            @if(isset($pbr)) @method('PUT') @endif

            <div class="card mb-3">
                <input type="hidden" id="checkHPP" value="0">
                <div class="card-header">{{__('Sales Invoice')}} {{__('Information')}} : <strong>{{$pbr->pbr_number}}</strong> </div>
                <div class="card-body">
                    <div class="row">
                        <div class="col-md-4">
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="text" name="customer_code" id="customer_code" class="form-control" readonly required value="{{ old('customer_code', $pbr->customer_code) }}">
                                </div>
                                <br>
                                <div class="form-group">
                                    <label for="customer_name">{{__('Customer Name')}}</label>
                                    <input type="text" name="customer_name" id="customer_name" class="form-control" readonly value="{{ old('customer_name', $pbr->customers->customer_name) }}" required>
                                </div>
                                <br>
                                <div class="form-group">
                                    <label for="address">{{__('Address')}}</label>
                                    <input type="text" name="address" id="address" class="form-control" value="{{ old('customer_name', $pbr->customers->address) }}" readonly>
                                </div>
                            {{-- <div class="form-group">
                                <label for="pbr_number">{{__('Sales Invoice Number')}}</label>
                                <input type="text" name="pbr_number" class="form-control" readonly required value="{{ old('pbr_number', $pbr->pbr_number ?? $pbr_number) }}">
                            </div> --}}
                            <br>
                        ...
                        <div class="col-md-4">
                            <div class="form-group">
                                ...
                                <textarea name="notes" class="form-control" rows="5" value="{{ old('notes', $pbr->notes) }}">{{ old('notes', $pbr->notes) ?? ''}}</textarea>

                            </div>
                            <br>
                            <div class="form-group">
                                <label for="document_date">{{__('Document Date')}}</label>
                                <input type="date" name="document_date" id= "document_date" class="form-control date-picker" required value="{{ old('document_date', $pbr->document_date ?? date('Y-m-d')) }}">
                            </div>
                            <br>
                            <div class="form-group">
                                <label for="delivery_date">{{__('Delivery Date')}}</label>
                                <input type="date" name="delivery_date" class="form-control date-picker" required value="{{ old('delivery_date', $pbr->delivery_date ?? date('Y-m-d')) }}">
                            </div>
                            <br>
                            <div class="form-group">
                                <label for="due_date">{{__('Due Date')}} Piutang</label>
                                <input type="date" name="due_date" class="form-control date-picker" required value="{{ old('due_date', $pbr->due_date ?? date('Y-m-d')) }}">
                            </div>
                        </div>
                        ...
                        <div class="col-md-4">
                            ...
                            <div class="form-group d-none">
                                ...
                                <input type="text" oninput="formatNumber(this)" id="disc_nominal" name="disc_nominal" class="form-control text-end nominal" required value="{{ old('disc_nominal', number_format($pbr->disc_nominal,0,'.',','))}}">
                            </div>
                            <br>
                            {{-- <div class="form-group">
                            ...
                            <div class="form-group">
                                ...
                                <div class="input-group mb-2">
                                    ...
                                    <select hidden class="form-select" id="tax" name="tax">
                                        @foreach ($taxs as $tax)
                                            <option value="{{ $pbr->tax }}" {{ old('tax', $pbr->tax) === $tax->tax_code ? 'selected' : '' }}>
                            ...
                            <div class="form-group">
                                ...
                                <div class="input-group mb-3">
                                    <select class="form-select" id="tax_revenue" name="tax_revenue">
                                        ...
                                        @foreach ($taxs as $tax)
                                            <option value="{{ $tax->tax_code }}" {{ $pbr->tax_revenue_tariff == $tax->tax_code ? 'selected' : '' }}>
                                                {{ $tax->tax_name . ' (' . $tax->tax_code . ')' }}
                                            </option>
                                        @endforeach
                                    </select>
                ...
                <div class="card-body">
                    ...
                    <table class="table" id="dynamicTable">
                        ...
                        <tbody id="parentTbody">
                            @foreach ($pbr->details as $index => $detail)
                            <tr>
                                <td>
            ...
            <div class="form-group submit-btn mb-3">
                <button type="button" onclick="confirmEdit(event,'{{ $pbr->id }}')" class="btn btn-primary" @if(!in_array('update', $privileges)) disabled @endif>Update {{__('Sales Invoice')}}</button>
            </div> @endif
            <a href="{{route('transaction.pbr')}}" class="btn btn-secondary mb-3">Back</a>
        </form>
        @if($editable)
        <form id="delete-form" action="{{ route('transaction.pbr.destroy', $pbr->id) }}" method="POST" style="display:inline;" >
            @csrf
            @method('POST')
            <input type="hidden" name="reason" id="deletion-reason">
            <button type="button" class="btn btn-sm btn-danger mb-3" onclick="confirmDelete(event,'{{ $pbr->id }}')"
...
<script>
    Swal.fire({
        ...
        if (result.isConfirmed) {
            window.location.href = "{{ route('transaction.pbr') }}";
        }
    });
</script>
@endif
...
let rowCount = {{ isset($pbr) ? count($pbr->details) : 1 }};
