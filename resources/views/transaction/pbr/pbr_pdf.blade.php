...
<head>
    <meta charset="UTF-8">
    <title>{{__('Sales Invoice')}} - {{ $pbr->pbr_number }}</title>
    <style>
...
<pre>
  ...
  TDS                               NOTA SALES PBR
  Kepada Yth.
  {{ str_pad($pbr->customers->customer_name ?? 'N/A', 35) }}No. PBR     : {{ $pbr->pbr_number }}
  {{ str_pad($pbr->customers->city?? 'N/A', 35) }}Tanggal     : {{ \Carbon\Carbon::parse($pbr->document_date)->format('d M Y') }}
...
@endphp
  {{ str_pad($terbilangLines[0], 35) }}                    Sub Total    : {{ str_pad(number_format($pbr->subtotal+$totalDiscount, 0, ',', '.'), 15, ' ', STR_PAD_LEFT) }}
  {{ str_pad($terbilangLines[1], 35) }}                    Discount     : {{ str_pad(number_format($totalDiscount, 0, ',', '.'), 15, ' ', STR_PAD_LEFT) }}
  {{ str_pad($terbilangLines[2], 35) }}                    PPN          : {{ str_pad(number_format($pbr->add_tax, 0, ',', '.'), 15, ' ', STR_PAD_LEFT) }}
@if(isset($terbilangLines[3]))
@foreach(array_slice($terbilangLines, 3) as $extraLine)
 {{ str_pad($extraLine, 35) }}
@endforeach
@endif
  {{ str_pad('', 35) }}                    Total Invoice: {{ str_pad(number_format($pbr->total, 0, ',', '.'), 15, ' ', STR_PAD_LEFT) }}
        </pre>
