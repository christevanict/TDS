...
@section('content')
    ...
    <div class="container content">
        ...
        <form id="po-form" action="{{ route('transaction.reimburse.update', $reimburse->id) }}" method="POST">
            @csrf
            @method('PUT')
            <div class="card mb-3">
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            <div class="form-group">
                                <label for="customer_code">Reimbursement Number</label>
                                <input type="text" name="customer_code" id="customer_code" value="{{$reimburse->reimburse_number}}" class="form-control" readonly >
                            </div>
                            <div class="form-group">
                                <label for="customer_code">{{__('Customer Code')}}</label>
                                <input type="text" name="customer_code" id="customer_code" value="{{$customers->customer_code}}" class="form-control" readonly >
                            </div>
                            <div class="form-group">
                                <label for="customer_name">{{__('Customer Name')}}</label>
                                <input type="text" name="customer_name" id="customer_name" value="{{$customers->customer_name}}" class="form-control" readonly>
                            </div>
                            <div class="form-group">
                                <label for="address">{{__('Address')}}</label>
                                <input type="text" name="address" id="address" value="{{$customers->address}}" class="form-control" readonly>
                            </div>
                        </div>
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <input type="text" id="sales_order_number" name="sales_order_number" class="form-control" required
                                   value="{{ $reimburse->contract_document_number }}" readonly>
                            ...
                            <div class="form-group">
                                ...
                                <input type="date" id="document_date" name="document_date" class="form-control date-picker" required
                                    value="{{ $reimburse->document_date  }}" readonly>
                            </div>
                            <div class="form-group">
            ...
            <div class="card mb-3">
                ...
                <div class="card-body">
                    <table class="table" id="po-details-table">
                        ...
                        <tbody>
                            @foreach ($reimburseDetails as $rowCount=>$detail)
                            <tr class= "trh" data-row-id="{{$rowCount}}">
                                <td>
                                    <input type="text" id="item_{{$rowCount}}" name="details[{{$rowCount}}][item_description]" class="qtyw form-control" readonly value="{{$detail->item_description}}">
                                </td>
                                <td>
                                    <input type="number" id="qty_{{$rowCount}}" name="details[{{$rowCount}}][price]" class="qtyw form-control" readonly value="{{$detail->price}}" >
                                </td>
                                <td>
                                    <select class="form-select" id="account_number_{{$rowCount}}" name="details[{{$rowCount}}][account_number]">
                                        @foreach ($coas as $coa)
                                            <option value={{$coa->account_number}}
                                                @if($detail->account_number ==$coa->account_number)
                                                selected
                                                @endif
                                                >{{$coa->account_number.' '.$coa->account_name}}</option>
                                        @endforeach
                                    </select>
                                </td>
                                <td>
                                    <input type="text" name="details[{{$rowCount}}][sales_invoice_number]" id="price_{{$rowCount}}" class="form-control" value="{{$detail->sales_invoice_vendor}}" required>
                                </td>
            ...
            <button type="submit" class="mb-3 btn btn-primary">Edit Reimburse</button>
        </form>

        <div class="row">
            <div class="col-sm">
                <form id="print-form" action="{{ route('transaction.reimburse.print', $reimburse->id) }}" method="GET" style="display:inline;">
                    <button type="submit" class="btn btn-dark mb-3">
                        Print</button>
                </form></div>
        <div class="col-sm-1">
        <form id="delete-form" action="{{ route('transaction.reimburse.destroy', $reimburse->id) }}" method="POST" style="display:inline;" >
            @csrf
            @method('POST')
            <button type="button" class="btn btn-sm btn-danger mb-3" onclick="confirmDelete(event,'{{ $reimburse->id }}')"
...
function confirmDelete(event, id) {
        ...
        Swal.fire({
            title: 'Are you sure?',
            text: 'Do you want to delete this reimburse?',
            icon: 'warning',
