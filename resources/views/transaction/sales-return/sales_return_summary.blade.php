...
@section('content')
    <x-page-title title="{{__('Sales Invoice')}} Summary" pagetitle="{{__('Sales Invoice')}} Summary {{__('Information')}}" />
    <hr>
    ...
    <div class="card">
        <div class="card-body">
            <h6 class="mb-2 text-uppercase">{{__('Sales Invoice')}} Summary</h6>
                    ...
                    <tbody>
                        @foreach ($salesInvoices as $invoice)
                            <tr class="clickable-row">
                                ...
                                <td>{{ $invoice->sales_return_number }}</td>
                                <td>{{ \Carbon\Carbon::parse($invoice->document_date)->format('d M Y') }}</td>
                                @if(in_array('price', $privileges))
                                <td>{{ number_format($invoice->disc_nominal, 2) }}</td>
                                <td>{{ number_format($invoice->subtotal, 2) }}</td>
                                <td>{{ number_format($invoice->tax_revenue, 2) }}</td>
                                <td>{{ number_format($invoice->add_tax, 2) }}</td>
