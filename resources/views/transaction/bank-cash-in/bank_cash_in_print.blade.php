...
<head>
    <meta charset="UTF-8">
    <title>{{__('Bank Cash In')}} - {{ $bankCashIn->bank_cash_in_number }}</title>
    <style>
        body {
            margin: 0;
            padding: 0;
...
<body>
    ...
    Paid From : {{ str_pad($bankCashIn->account_number.'-'.$bankCashIn->coa->account_name ?? 'N/A', 35) }}Voucher No. {{$bankCashIn->bank_cash_in_number}}
    Tanggal   : {{ \Carbon\Carbon::parse($bankCashIn->bank_cash_in_date)->format('d M Y') }}
...
@php
    ...
    $totalPayment = 'Total Payment: '.number_format($bankCashIn->nominal, 0);
...
@endforeach
    ...
    Tgl.                        Tgl.                        Tgl.
        ...
        <a class="btn-print" href="{{route('transaction.bank_cash_in')}}">Kembali</a>
    </div>
</body>
</html>
