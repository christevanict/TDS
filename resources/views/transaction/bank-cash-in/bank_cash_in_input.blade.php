...
@section('content')
<div class="row">
    <x-page-title title="{{__('Bank Cash In')}}" pagetitle="{{__('Bank Cash In')}}" />
    <hr>
    <div class="container content">
        <h2>{{__('Bank Cash In')}} Input</h2>



        <form id="bank-cash-in-form" action="{{ isset($bankCashIn) ? route('transaction.bank_cash_in.update', $bankCashIn->id) : route('transaction.bank_cash_in.store') }}" method="POST">
            @csrf
            @if(isset($bankCashIn)) @method('PUT') @endif
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            {{-- <div class="form-group">
                                ...
                                <input type="text" name="bank_cash_in_number" class="form-control" readonly required value="{{ old('bank_cash_in_number', $bankCashIn->bank_cash_in_number ?? $bank_cash_in_number) }}">
                            </div> --}}
                            <div class="form-group">
                                <label for="bank_cash_in_date">Tanggal {{__('Bank Cash In')}}</label>
                                <input type="date" name="bank_cash_in_date" class="form-control date-picker" required value="{{ old('bank_cash_in_date', $bankCashIn->bank_cash_in_date ?? date('Y-m-d')) }}" id="document_date">
                            </div>
<br>
                            <div class="form-group">
            ...
            <div class="form-group submit-btn mb-3">
                <button type="submit" class="btn btn-success" @if(!in_array('create', $privileges)) disabled @endif>Submit Kas Masuk</button>
            </div>
        </form>
    </div>
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        ...
                        if (id) {
                            ...
                            window.location.href = "{{ route('transaction.bank_cash_in.print', ['id' => ':id']) }}".replace(':id', id);
                        }
                    }
                });
            @endif
