...
@section('title', 'Edit Sales Credit Debit Note')
...
@section('content')
<div class="row">
    <x-page-title title="Sales Credit Debit Note" pagetitle="Edit Sales Credit Debit Note" />
    <hr>
    <div class="container content">
        <h2>Edit Sales Credit Debit Note</h2>

        <form id="sales-credit-debt-note-form" action="{{ route('transaction.sales_debt_credit_notes.update', $salesDebtCreditNote->id) }}" method="POST">
            @csrf
            @method('PUT')

            <div class="card mb-3">
                <div class="card-header">Sales Credit Debit Note {{__('Information')}}</div>
                <div class="card-body">
                    <div class="row">
                        <div class="col-md-6">
                            <div class="form-group mt-3">
                                <label for="sales_credit_note_date">Debit/Credit Note Date</label>
                                <input type="date" name="sales_credit_note_date" id="sales_credit_note_date" class="form-control date-picker" required value="{{ $salesDebtCreditNote->sales_credit_note_date }}">
                            </div>

                            <div class="form-group mt-3">
                                <label for="invoice_number">Invoice Number</label>
                                <input type="text" name="invoice_number" id="invoice_number" class="form-control" required value="{{ $salesDebtCreditNote->invoice_number }}" readonly>
                                <input type="hidden" name="total_old" id="total_old" class="form-control" required value="{{ $salesDebtCreditNote->total }}">
                            </div>
                        </div>
                        <div class="col-md-6">
                            <div class="form-group mt-3">
                                <label for="account_receivable">Account Receivable</label>
                                <div class="form-group mb-3">
                                    <div class="input-group">
                                        <input type="text" id="search-acc-receivable" class="form-control" autocomplete="off" placeholder="Search by Account Number or Account Name" required readonly value="{{$salesDebtCreditNote->account_receivable.' - '.$coas->firstWhere('account_number',$salesDebtCreditNote->account_receivable)->account_name}}" >
                                        <button style="height:100%;" class="btn btn-outline-secondary" type="button" onclick="clearInput('search-acc-receivable')"><i class="material-icons-outlined">edit</i></button>
                                    </div>
                                    ...
                                    <input type="hidden" name="account_receivable" id="account_receivable" value="{{$salesDebtCreditNote->account_receivable}}">
                                </div>
                            </div>
            ...
            <div class="card mb-3">
                <div class="card-header">Debit/Credit Note Details</div>
                <div class="card-body">
                    <table class="table table-bordered" id="details-table">
                        ...
                        <tbody>
                            @if($salesDebtCreditNote->details)
                                @foreach ($salesDebtCreditNote->details as $index => $detail)
                                <tr class="detail-row">
                                    <td>
                                        <div class="form-group mb-3">
                                            <div class="input-group">
        ...
        <form id="delete-form" action="{{ route('transaction.sales_debt_credit_notes.destroy', $salesDebtCreditNote->id) }}" method="POST" style="display:inline;" >
            @csrf
            @method('DELETE')
            <input type="hidden" name="total_old" id="total_old" class="form-control" required value="{{ $salesDebtCreditNote->total }}">
            <input type="hidden" name="status" id="status" class="form-control" required value="{{ $salesDebtCreditNote->status }}">
            <input type="hidden" name="order" id="order" class="form-control" required value="{{ $salesDebtCreditNote->invoice_number }}">
            <button type="button" class="btn btn-sm btn-danger mb-3" onclick="confirmDelete(event,'{{ $salesDebtCreditNote->id }}')"
...
$('#sales_credit_note_date').prop('max', maxDate);
    ...
    let detailIndex = {{ $salesDebtCreditNote->details ? count($salesDebtCreditNote->details) : 0 }};
    ...
    function confirmDelete(event, id) {
        ...
        Swal.fire({
            title: 'Are you sure?',
            text: 'Do you want to delete this sales debet/credit note?',
            icon: 'warning',
