...
@section('title', 'Input Sales Credit Note')
...
@section('content')
<div class="row">
    <x-page-title title="Sales Credit Note" pagetitle="Sales Credit Note Input" />
    <hr>
    <div class="container content">
        <h2>Sales Credit Note Input</h2>

        <form id="sales-credit-note-form" action="{{ route('transaction.sales_debt_credit_notes.store') }}" method="POST">
            @csrf


            <input type="hidden" name="status" value="credit">
            <div class="card mb-3">
                <div class="card-header">Sales Credit Note {{__('Information')}}</div>
                <div class="card-body">
                    <div class="row">
                        <div class="col-md-6">
                            <div class="form-group mt-3">
                                <label for="sales_credit_note_date">Credit Note Date</label>
                                <input type="date" name="sales_credit_note_date" id="sales_credit_note_date" class="form-control date-picker" required value="{{ date('Y-m-d') }}" >
                            </div>
                            <div class="form-group mt-3">
                                ...
                                <select name="invoice_number" id="invoice_number" class="form-control" required>
                                    ...
                                    @foreach($salesInvoices as $invoice)
                                        <option value="{{ $invoice->sales_invoice_number }}">{{ $invoice->sales_invoice_number }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                            ...
                            <div class="form-group mt-3">
                                ...
                                <div class="form-group mb-3">
                                    <div class="input-group">
                                        <input type="text" id="search-acc-receivable" class="form-control" autocomplete="off" placeholder="Search by Account Number or Account Name" required >
