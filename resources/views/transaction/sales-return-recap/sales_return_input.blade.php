...
@section('content')
    ...
    <div class="container content">
        <h2>REKAP {{__('Sales Return')}} Transaction</h2>
        <form id="po-form" action="{{ route('transaction.sales_return_recap.store') }}" method="POST">
            @csrf
            <div class="card mb-3">
                <div class="card-header">REKAP {{__('Sales Return')}} {{__('Information')}}</div>
                <div class="card-body">
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        ...
                        if (id) {
                            ...
                            window.open("{{ route('transaction.sales_return_recap.print', ['id' => ':id']) }}".replace(':id', id), '_blank');
                        }
                    }
                });
            @endif
...
function calculateTotals() {
    ...
    document.getElementById('department_code').addEventListener('change', function() {
        ...
        $.ajax({
            url: "{{ route('transaction.sales_return_recap.changeDepartment') }}",
            method: 'POST',
            data: {
