...
<head>
    <meta charset="UTF-8">
    <title>{{__('Purchase Return')}} - {{ $purchaseInvoice->purchase_invoice_number }}</title>
    <style>
...
<body>
  ...
  TDS                      RETUR PETDSLIAN
  Kepada Yth.
  {{ str_pad($purchaseInvoice->suppliers->supplier_name ?? 'N/A', 35) }}No. Retur : {{ $purchaseInvoice->purchase_return_number }}
...
@if($lineIndex == 0)
 |{{ str_pad($loop->parent->iteration . '.', 3) }}|{{ str_pad($line, 35) }}|{{ str_pad(number_format($detail->qty, 2), 5, ' ', STR_PAD_LEFT) }}|{{ str_pad(number_format($detail->base_qty * $detail->qty, 2, ',', '.') . ' ' . $detail->baseUnit->unit_name, 10, ' ', STR_PAD_LEFT) }}|{{ str_pad(number_format($detail->price, 0, ',', '.'), 12, ' ', STR_PAD_LEFT) }}|{{ str_pad(number_format($detail->nominal, 0, ',', '.'), 15, ' ', STR_PAD_LEFT) }}|
@else
 |{{ str_pad('', 3) }}|{{ str_pad($line, 35) }}|{{ str_pad('', 5) }}|{{ str_pad('', 10) }}|{{ str_pad('', 12) }}|{{ str_pad('', 15) }}|
@endif
@endforeach
@endforeach
 ---------------------------------------------------------------------------------------
...
@endphp
  {{ str_pad($terbilangLines[0], 35) }}                    Sub Total    : {{ str_pad(number_format($purchaseInvoice->subtotal, 0, ',', '.'), 15, ' ', STR_PAD_LEFT) }}
  {{ str_pad($terbilangLines[1], 35) }}                    Discount     : {{ str_pad(number_format($purchaseInvoice->disc_nominal, 0, ',', '.'), 15, ' ', STR_PAD_LEFT) }}
  {{ str_pad($terbilangLines[2], 35) }}                    PPN          : {{ str_pad(number_format($purchaseInvoice->add_tax, 0, ',', '.'), 15, ' ', STR_PAD_LEFT) }}
...
@endif
  {{ str_pad('', 35) }}                    Total Invoice: {{ str_pad(number_format($purchaseInvoice->total, 0, ',', '.'), 15, ' ', STR_PAD_LEFT) }}
        ...
        <a class="btn-print" href="{{route('transaction.purchase_return')}}">Kembali</a>
    </div>
</body>
</html>
