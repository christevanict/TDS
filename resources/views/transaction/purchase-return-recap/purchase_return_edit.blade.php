...
@section('content')
    ...
    <div class="container content">
        ...
        <form id="print-form" action="{{ route('transaction.purchase_return_recap.print', $purchaseReturn->id) }}" target="_blank" method="GET" style="display:inline;">
            <button type="submit" class="btn btn-dark mb-3" @if(!in_array('print', $privileges)) disabled @endif>
                Print
            </button>
        </form>
        <form id="po-form" action="{{ route('transaction.purchase_return_recap.update', $purchaseReturn->id) }}" method="POST">
            @csrf
            @method('PUT')
            <div class="card mb-3">
...
{{--
        ...
        <form id="delete-form" action="{{ route('transaction.purchase_return_recap.destroy', $purchaseReturn->id) }}" method="POST" style="display:inline;" >
            @csrf
            @method('POST')
        ...
        <a class=" mb-3 btn btn-secondary" href="{{route('transaction.purchase_return_recap')}}">Back</a>
    </div>
