...
@section('content')
    <x-page-title title="Transaction" pagetitle="REKAP {{__('Purchase Invoice')}}" />
    <hr>
    <div class="container content">
        <h2>{{__('Purchase Invoice')}} Edit</h2>
        <form id="print-form" action="{{ route('transaction.purchase_invoice_recap.print', $purchaseOrder->id) }}" target="_blank" method="GET" style="display:inline;">
            <button type="submit" class="btn btn-dark mb-3" @if(!in_array('print', $privileges)) disabled @endif>
                Print PI
            </button>
        </form>
        <form id="print-form" action="{{ route('transaction.purchase_invoice_recap.print.netto', $purchaseOrder->id) }}" target="_blank" method="GET" style="display:inline;">
            <button type="submit" class="btn btn-dark mb-3" @if(!in_array('print', $privileges)) disabled @endif>
                Print PI Netto
            </button>
        </form>
        <form id="po-form" action="{{ route('transaction.purchase_invoice_recap.update', $purchaseOrder->id) }}" method="POST">
            @csrf
            @method('PUT')
            <div class="card mb-3">
                <div class="card-header">REKAP {{__('Purchase Invoice')}} {{__('Information')}}</div>
                @if (!$editable)
...
{{--
            ...
            <div class="card mb-3">
                ...
                <a class=" mb-3 btn btn-secondary" href="{{route('transaction.purchase_invoice_recap')}}">Back</a>
        </form>
        {{-- @if($purchaseOrder->status!='Cancelled'&&$purchaseOrder->status!='Closed')
        <form id="cancel-form" action="{{ route('transaction.purchase_invoice_recap.cancel', $purchaseOrder->id) }}" method="POST" style="display:inline;" >
            @csrf
            @method('POST')
        ...
        @if($editable)
            <form id="delete-form" action="{{ route('transaction.purchase_invoice_recap.destroy', $purchaseOrder->id) }}" method="POST" style="display:inline;" >
                @csrf
                @method('POST')
